...
public class UserRecommendService {
                ...
                userFollowRepository.findAll().stream()
                                ...
                                .forEach((userId, follows) -> {
                                        if (!userId.equals(currentUserId)) {
                                                ...
                                                double similarity = calculateJaccardSimilarity(currentUserFollowing,
                                                                userFollowing);
                                ...
                                .forEach((userId, favorites) -> {
                                        if (!userId.equals(currentUserId)) {
                                                Set<String> userTags = favorites.stream()
                                                                .map(favorite -> tagRepository
                                                                                .findByPostId(favorite.getPostId())
        ...
        public List<User> recommendUsers(LoginResponseDto loginInfo, int k) {
                ...
                // 合并邮箱和位置得分
                emailScores.forEach((userId, score) -> finalScores.merge(userId, score * 0.2, Double::sum));

                locationScores.forEach((userId, score) -> finalScores.merge(userId, score * 0.3, Double::sum));

                // 添加关注和收藏相似度得分
                calculateFollowOverlapScore(currentUser.getUserId())
                                .forEach((userId, score) -> finalScores.merge(userId, score * 0.25, Double::sum));

                calculateFavoritesSimilarityScore(currentUser.getUserId())
                                .forEach((userId, score) -> finalScores.merge(userId, score * 0.25, Double::sum));

                // 如果没有任何推荐结果，返回最新注册的活跃用户
                if (finalScores.isEmpty()) {
                        return userRepository.findTop5ByIsDeletedFalseOrderByLastLoginTimeDesc();
                }
                ...
                return k >= recommendedUsers.size() ? recommendedUsers : recommendedUsers.subList(0, k);
        }
