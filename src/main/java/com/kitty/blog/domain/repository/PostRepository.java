...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query("SELECT CASE WHEN COUNT(p) > 0 " +
            "THEN true ELSE false END FROM Post p WHERE p.postId = ?1")
     ...
     * @return
     */
    @Query("SELECT p FROM Post p WHERE p.isPublished = ?1 AND p.userId = ?2")
     ...
     * @return
     */
    @Query("SELECT p FROM Post p JOIN p.postTags pt WHERE pt.id.tagId = ?1")
